...
if ($_POST['salvar'])
{
	...
	else
	{
		...
		else
		{
			...
			$builder->setWhere("nome='{$nome}'");
			$conn->sql_query($builder->buildQuery());
			if ($conn->sql_numrows()) $error='Inscrição já cadastrada!';
			else
			{
...
$builder->addColumn('mod_cursos.code');
$builder->addColumn('nome_curso');
$builder->setJoin("INNER JOIN public.mod_turmas ON (public.mod_cursos.code = public.mod_turmas.curso_code)");
...
$smarty->display('mod_inscricoes-cursos.htm');
?>